...
class Bullet {
   ...
   */
  init(ownerId, x, y, vx, vy, radius = 4, ttl = 3) {
    this.id      = `b${_nextId++}`;
 ...
 */
function acquireBullet(ownerId, x, y, vx, vy, radius, ttl) {
  const start = _poolIdx;
  do {
    const b = _pool[_poolIdx];
    _poolIdx = (_poolIdx + 1) % POOL_SIZE;
    if (!b.active) return b.init(ownerId, x, y, vx, vy, radius, ttl);
  } while (_poolIdx !== start);

  console.warn('[BulletPool] Pool exhausted — allocating outside pool');
  return new Bullet().init(ownerId, x, y, vx, vy, radius, ttl);
}
