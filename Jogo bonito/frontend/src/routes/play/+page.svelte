<script lang="ts">
  ...
  import { GameRuntimeSession, readSessionQuery, type Difficulty, type PlayerRole } from '$lib/game/session/gameRuntimeSession';
  ...
  let currentDifficultyLabel = $state('Normal');
  let currentWorld = $state(1);
  let currentStage = $state(1);
  let stageClearTitle = $state('STAGE CLEARED!');
  let stageClearSubtext = $state('Boss derrotado.');
  let canAdvanceStage = $state(false);
  let isCampaignComplete = $state(false);
  ...
  function handleStageClearAction() {
    if (!session) return;
    if (canAdvanceStage) {
  ...
  let stageClearMetaText = $derived(
    `Dificultad: ${currentDifficultyLabel} | Mundo ${currentWorld} | Stage ${currentStage}`
  );

  let stageClearButtonText = $derived(
    canAdvanceStage ? '▶ SIGUIENTE STAGE' : (isCampaignComplete ? '🏆 REINICIAR CAMPANA' : '🔄 JUGAR OTRA VEZ (REPLAY)')
  );
  ...
  onMount(() => {
    const params = readSessionQuery(window.location.search, 'normal');
    ...
    session = new GameRuntimeSession({
      ...
      onEngineFrame: (engine) => {
        ...
        currentDifficultyLabel = engine.currentDifficultyLabel;
        currentWorld = engine.currentWorld;
        currentStage = engine.currentStage;
        stageClearTitle = engine.stageClearTitle;
        stageClearSubtext = engine.stageClearSubtext;
        canAdvanceStage = engine.canAdvanceStage;
        isCampaignComplete = engine.isCampaignComplete;
    ...
    <main class="combat-arena-wrapper">
      ...
      <div class="canvas-container">
        ...
        <GameArenaOverlays
          ...
          {stageClearTitle}
          {stageClearSubtext}
          stageClearMeta={stageClearMetaText}
          stageClearButtonText={stageClearButtonText}
          onPushStruggle={handlePushStruggle}
          onRestartStage={handleStageClearAction}
        />
